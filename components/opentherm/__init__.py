...
cv.Optional(CONF_BURNER_OPERATION_HOURS): sensor.sensor_schema(
    ...
    accuracy_decimals=0,
    state_class=STATE_CLASS_TOTAL_INCREASING).extend(),
...
cv.Optional(CONF_SOLAR_STORAGE_TEMPERATURE): sensor.sensor_schema(
    ...
    accuracy_decimals=1,
    state_class=STATE_CLASS_MEASUREMENT).extend(),
cv.Optional(CONF_IS_CH2_ACTIVE): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
cv.Optional(CONF_IS_CH_ACTIVE): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
cv.Optional(CONF_IS_COOLING_ACTIVE): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
cv.Optional(CONF_IS_DHW_ACTIVE): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
cv.Optional(CONF_IS_DIAGNOSTIC_EVENT): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
cv.Optional(CONF_IS_FAULT_INDICATION): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
cv.Optional(CONF_IS_FLAME_ON): binary_sensor.binary_sensor_schema(device_class=DEVICE_CLASS_EMPTY).extend(),
})
...
def to_code(config):
    ...
    cg.add(var.set_dhw_pump_valve_operation_hours(config[CONF_DHW_PUMP_VALVE_OPERATION_HOURS]))
    ...
    cg.add(var.set_solar_collector_temperature(config[CONF_SOLAR_COLLECTOR_TEMPERATURE]))
