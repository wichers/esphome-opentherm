...
// #6: Remote-parameter
void OpenThermGWClimate::process_Slave_MSG_RBP_FLAGS(const uint32_t response) {
    uint8_t ub = getUBUInt8(response);
    uint8_t lb = getLBUInt8(response);
...
void OpenThermGWClimate::process_Slave_MSG_TDHWSET_UB_LB(const uint32_t response) {
    int8_t dhw_setpoint_ub = getUBInt8(response); // Upper bound for adjustment of DHW setp (°C)
    int8_t dhw_setpoint_lb = getLBInt8(response); // Lower bound for adjustment of DHW setp (°C)
}

// #49: Max CH water setpoint upper & lower bounds for adjustment (°C)
void OpenThermGWClimate::process_Slave_MSG_MAXTSET_UB_LB(const uint32_t response) {
...
void OpenThermGWClimate::process_Master_MSG_TDHWSET(const uint32_t request) {
...
// #56: DHW setpoint (°C) (Remote parameter 1)
void OpenThermGWClimate::process_Slave_MSG_TDHWSET(const uint32_t request) {
    float dhw_setpoint = getFloat(request); // Domestic hot water temperature setpoint (°C)
    ESP_LOGD(TAG, "slave dhw_setpoint: %f", dhw_setpoint);
}

// #57: Current room setpoint for 2nd CH circuit (°C)
void OpenThermGWClimate::process_Master_MSG_MAXTSET(const uint32_t request) {
...
// #57: Current room setpoint for 2nd CH circuit (°C)
void OpenThermGWClimate::process_Slave_MSG_MAXTSET(const uint32_t request) {
    float max_ch_water_setpoint = getFloat(request); //  Maximum allowable CH water setpoint (°C)
...
void OpenThermGWClimate::process_Slave_MSG_TSP(const uint32_t response) {
...
void OpenThermGWClimate::process_Master_MSG_TSP_INDEX_TSP_VALUE(const uint32_t response) {
...
// #11: Index number / Value of referred-to transparent slave parameter.
void OpenThermGWClimate::process_Slave_MSG_TSP_INDEX_TSP_VALUE(const uint32_t response) {
    uint8_t tsp_index_no = getUBUInt8(response); // Index number of following TSP
    uint8_t tsp_value = getLBUInt8(response); // Value of above referenced TSP
}
...
void OpenThermGWClimate::process_Slave_MSG_FHB_SIZE(const uint32_t response) {
...
void OpenThermGWClimate::process_Slave_MSG_FHB_INDEX_FHB_VALUE(const uint32_t response) {
...
void OpenThermGWClimate::process_Master_MSG_COOLING_CONTROL(const uint32_t request) {
...
void OpenThermGWClimate::process_Master_MSG_MAX_REL_MOD_LEVEL_SETTING(const uint32_t request) {
    // Maximum relative boiler modulation level setting for sequencer and off-low & pump control applications.
    float max_rel_mod_level_setting = getFloat(request); 
    ESP_LOGD(TAG, "max_rel_mod_level_setting: %f", max_rel_mod_level_setting);
}

// #15: Maximum boiler capacity (kW) / Minimum boiler modulation level(%)
void OpenThermGWClimate::process_Slave_MSG_MAX_CAPACITY_MIN_MOD_LEVEL(const uint32_t response) {
